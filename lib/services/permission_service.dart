...
class PermissionService {
  ...
  Future<bool> checkBatteryExemption() async {
  ...
  Future<bool> ensureBatteryExemptionPermission(BuildContext context) async {
    if (await checkBatteryExemption()) {
      return true;
    }
    ...
    if (result.isPermanentlyDenied) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text(
              'Medication alarms may not be triggered properly. Please disable battery optimisation for this app in Settings.',
            ),
            action: SnackBarAction(
              label: 'Settings',
              onPressed: () => openAppSettings(),
